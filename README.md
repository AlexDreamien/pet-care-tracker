...
Короткая инструкция для тех, кто пользуется приложением, а не разрабатывает его.

### Регистрация

1. Откройте адрес приложения. Первый запрос после долгого простоя может занять
   10–20 секунд — сервер засыпает, пока никто не заходит.
2. На экране входа нажмите **«Создать аккаунт»**: e-mail, пароль и **код приглашения**.
   Если на сервере задан `SIGNUP_INVITE_CODE`, без кода регистрация закрыта — попросите
   его у владельца.
3. После регистрации будет показан **код восстановления**. Сохраните его: почты у сервиса
   нет, и это единственный способ войти, если забудете пароль. Показывается один раз.
4. На телефоне добавьте приложение на экран «Домой» (iOS: Поделиться → _На экран «Домой»_;
   Android: меню → _Установить приложение_). Так работают push-уведомления и просмотр
   карточки без сети. После входа приложение предложит добавить passkey — отпечаток или
   Face ID вместо пароля.

### Как пользоваться

Первый пользователь создаёт **домохозяйство**; питомцы принадлежат ему, а не аккаунту,
поэтому партнёра можно добавить участником.

- **Питомцы** (главный экран) → «Добавить»: имя, вид, дата рождения, номер чипа. Внутри
  карточки — вкладки:
  - **Профиль** — паспортные данные, фото, аллергии и хронические особенности. Кнопка
    «Открыть карточку» печатает экстренную карточку для клиники; ниже — QR-бирка на
    ошейник на случай потери.
  - **Медкнижка** — прививки, обработки от паразитов, лекарства, визиты. Следующая дата
    считается сама; обработки отсчитываются от фактического выполнения, а не от плана.
  - **Замеры** — вес и обхваты, график, целевой коридор.
  - **Питание** — открытая пачка корма и прогноз, когда она закончится.
  - **Анализы** — показатели относительно нормы с бланка лаборатории.
  - **Документы** — сканы с датой окончания; приложение напомнит заранее.
  - **Расписание** — свои повторяющиеся дела: когти, груминг, купание.
- **Сегодня** — всё просроченное и ближайшее по всем питомцам. «Отметить выполненным»
  сдвигает следующую дату.
- **Расходы** — траты по категориям и месяцам; корм и визиты попадают сюда сами.
- **Контакты** — клиника, грумер, передержка; «Позвонить» в одно касание.
- **Настройки** — язык, единицы, часовой пояс; **подписка на календарь** (скопируйте
  ссылку в Google или Apple Calendar — напоминания придут туда); **push-уведомления**;
  **участники** — партнёр как редактор или только для чтения; **ссылка для ситтера** —
  временная страница с аллергиями и дозами для того, кто присматривает за животным;
  экспорт всех данных.

Приложение хранит и напоминает. Оно не ставит диагнозов и не назначает дозировки —
встроенные графики прививок и обработок лишь заполняют дату напоминания, и их можно
менять.
